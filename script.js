document.addEventListener('DOMContentLoaded', () => {
        ...
        for (let row = 0; row < 8; row++) {
            for (let col = 0; col < 8; col++) {
                ...
                if (piece) {
                    ...
                    if ((row === 0 || row === 7) && !piece.evolved && piece.type !== 'king') {
                        pieceElement.classList.add('promoted');
                    }
                    if (piece.evolved && piece.evolutionType === 'duck') {
                        pieceElement.classList.add('duck-piece');
                    }
    ...
    function handleSquareClick(row, col) { 
        ...
        if (selectedSquare && boardState[row][col]?.evolutionType === 'duck') {
            return;
        }
        ...
        if (selectedSquare) {
            ...
            if (isValidMove(fromRow, fromCol, row, col)) {
                ...
                if (!fromPiece.evolved && (
                    ...
                    (fromPiece.type === 'queen' && fromPiece.moves === 2) ||
                    (fromPiece.type === 'king' && fromPiece.moves === 1) ||
                    (fromPiece.moves === 1 && !fromPiece.evolved && 
                    (fromRow === 0 || fromRow === 7))
                )) {
    ...
    function isPromotedPiece(piece, row) {
        return (row === 0 || row === 7) && 
               !piece.evolved && 
               piece.type !== 'pawn' && 
               piece.type !== 'king';
    ...
    function showEvolutionOptions(piece, row, col) {
        console.log('Showing evolution for:', piece.type); // Добавьте эту строку
    }
    ...
    function movePiece(fromRow, fromCol, toRow, toCol) {
        ...
        if (piece.type === 'pawn' && (toRow === 0 || toRow === 7)) {
            ...
            // Блокируем игру на время выбора
            gameActive = true;
            pendingEvolution = { row: toRow, col: toCol };
            showPawnPromotionOptions(toRow, toCol, piece);
            return;
        } else {
            // Для обычных ходов увеличиваем счётчик
            boardState[toRow][toCol] = {...piece, moves: piece.moves + 1};
            boardState[fromRow][fromCol] = null;
            
            // Проверяем эволюцию для превращённых фигур
            if (isPromotedPiece(piece, toRow) && piece.moves === 0) {
                pendingEvolution = { row: toRow, col: toCol };
                showEvolutionOptions(piece, toRow, toCol);
                return;
            }
        }
    ...
    function highlightValidMoves(row, col) {
        ...
        switch(pieceType) {
            ...
            case 'spearmen':
                ...
                break;
            case 'move+-spearmen':
                highlightMovePlusSpearmenMoves(row,col,piece);
                break;
            case 'move+ pawn':
    ...
    function isValidMove(fromRow, fromCol, toRow, toCol) {
        ...
        if (fromPiece.evolved && fromPiece.evolutionType === 'x-ray-bishop') {
            ...
            if (fromPiece.evolved && fromPiece.evolutionType === 'duck') {
                return !boardState[toRow][toCol]; // Можно ходить на любую пустую клетку
            }
        }
    ...
    function getPieceName(type) {
        const names = {
            ...
            'king': 'король',
            'spearmen': 'копейщик',
            'move+ spearmen': 'подвижный копейщик',
            'move+ pawn': 'пешка+',
    ...
    function highlightDuckMoves(row, col, piece) {
        // Подсвечиваем ВСЕ свободные клетки доски
        for (let r = 0; r < 8; r++) {
            for (let c = 0; c < 8; c++) {
                if (!boardState[r][c] && !(r === row && c === col)) {
                    highlightSquare(r, c, 'duck-move');
                }
            }
        }
    }
    ...
    function promotePawn(row, col, newType) {
        const pawn = boardState[row][col];
        boardState[row][col] = {
            type: newType,
            color: pawn.color,
            moves: pawn.moves,
            evolved: false // Явно указываем, что это не эволюция
        };
        
        // Разблокируем игру и сбрасываем состояние
        gameActive = true;
        pendingEvolution = null;
        renderBoard();
        
        // Не передаём ход, чтобы игрок мог сразу эволюционировать
        // switchPlayer(); // Убрали эту строку
    }


    function showPawnPromotionOptions(row, col, piece) {
        evolutionPanel.classList.remove('hidden');
        evolutionPanel.querySelector('h3').textContent = 'Превращение пешки';
        evolutionOptions.innerHTML = '';
        document.getElementById('game-info').classList.add('blurred');
        
        const options = [
            { type: 'knight', name: 'Конь', description: 'Ходит буквой Г' },
            { type: 'bishop', name: 'Слон', description: 'Ходит по диагонали' },
            { type: 'rook', name: 'Ладья', description: 'Ходит по прямой' },
            { type: 'queen', name: 'Ферзь', description: 'Ходит в любом направлении' }
        ];
        ...
        options.forEach(option => {
            ...
            optionElement.addEventListener('click', () => {
                promotePawn(row, col, option.type);
                evolutionPanel.classList.add('hidden');
            });
        ...
        if (piece.type === 'pawn') {
            ...
            options = [
                ...
                },
                {
                    type: 'duck',
                    name: 'Утка',
                    description: 'Можно переместить на любую свободную клетку. Несъедобная.'
                }
            ];
